...
public class OmsPromotionServiceImpl implements OmsPromotionService {
    ...
    public List<CartPromotionItem> calcCartPromotion(List<OmsCartItem> cartItemList) {
        ...
        for (Map.Entry<Long, List<OmsCartItem>> entry : productCartMap.entrySet()) {
            ...
            if (promotionType == 1) {
                ...
                PmsProductLadder ladder = getProductLadder(count, promotionProduct.getDiscountInfo().getProductLadderList());
                if(ladder!=null){
                    for (OmsCartItem item : itemList) {
                        CartPromotionItem cartPromotionItem = new CartPromotionItem();
                        BeanUtils.copyProperties(item,cartPromotionItem);
                ...
                PmsProductFullReduction fullReduction = getProductFullReduction(totalAmount,promotionProduct.getDiscountInfo().getProductFullReductionList());
    ...
    private PmsSkuStock getOriginalPrice(PromotionProduct promotionProduct, Long productSkuId) {
        for (PmsSkuStock skuStock : promotionProduct.getSkuStockInfo().getSkuStockList()) {
